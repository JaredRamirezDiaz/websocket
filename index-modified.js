    ...
    switch(data.type) {
        case MessageType.IDENTIFY:
            ...
            if (clientType === 'panel') {
                if (!masterPanelId) {
                    ...
                    // The new master will send its state with SYNC_REQUEST
                }
                
                // Always send a sync response, even to the new master,
                // so it knows if there was a pre-existing state or not.
                console.log(`Sending SYNC_RESPONSE to new panel ${clientId}.`);
        ...
        case MessageType.SYNC_REQUEST:
            if (clientId === masterPanelId) {
                ...
                // Notify other panels that the state has been updated.
                clients.forEach(client => {
        ...
        case MessageType.SERVICE_UPDATE:
             if (clients.get(clientId)?.type === 'panel') {
                ...
                broadcast(data, clientId, 'panel'); // Notify all panels
             }
            break;
  ...
  ws.on('close', () => {
    ...
    if (clientId === masterPanelId) {
        ...
        if (nextPanel) {
            ...
            // The new master should now send a SYNC_REQUEST with its state.
            // We can prompt it to do so, or just wait for its next action.
        } else {
